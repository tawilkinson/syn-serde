...
// -----------------------------------------------------------------------------
// Compact JSON functions (without span information)

/// Removes span information from a JSON value recursively.
/// 
/// This function traverses a JSON value and removes any objects with the key "span",
/// resulting in more compact JSON output that focuses on the syntax tree structure
/// without location information.
pub fn remove_spans(value: &mut Value) {
    match value {
...
/// Serialize the given [`Syn`] type as compact JSON into the I/O stream.
/// 
/// This function produces JSON output without span information, making it more
/// compact and easier to read when location data is not needed.
///
/// # Examples
///
/// ```
...
/// json::to_writer_compact(&mut output, &syn_file)?;
/// let json_str = String::from_utf8(output).unwrap();
/// assert!(!json_str.contains("span"));
/// # Ok(())
/// # }
/// ```
pub fn to_writer_compact<S, W>(writer: W, syn: &S) -> Result<()>
where
    S: Syn,
    W: io::Write,
{
    let adapter = syn.to_adapter();
    let mut value = serde_json::to_value(&adapter)?;
    remove_spans(&mut value);
    serde_json::to_writer(writer, &value)
}

/// Serialize the given [`Syn`] type as pretty-printed compact JSON into the I/O stream.
/// 
/// This function produces pretty-printed JSON output without span information.
pub fn to_writer_compact_pretty<S, W>(writer: W, syn: &S) -> Result<()>
where
    S: Syn,
    W: io::Write,
{
...
/// Serialize the given [`Syn`] type as compact JSON byte vector.
/// 
/// This function produces JSON output without span information.
#[allow(clippy::missing_panics_doc)]
pub fn to_vec_compact<S>(syn: &S) -> Vec<u8>
where
    S: Syn,
{
    let adapter = syn.to_adapter();
...
/// Serialize the given [`Syn`] type as pretty-printed compact JSON byte vector.
/// 
/// This function produces pretty-printed JSON output without span information.
#[allow(clippy::missing_panics_doc)]
pub fn to_vec_compact_pretty<S>(syn: &S) -> Vec<u8>
where
    S: Syn,
{
    let adapter = syn.to_adapter();
...
/// Serialize the given [`Syn`] type as compact JSON string.
/// 
/// This function produces JSON output without span information, making it more
/// compact and easier to read when location data is not needed.
///
...
/// ```
#[allow(clippy::missing_panics_doc)]
pub fn to_string_compact<S>(syn: &S) -> String
where
    S: Syn,
{
    let adapter = syn.to_adapter();
...
/// Serialize the given [`Syn`] type as pretty-printed compact JSON string.
/// 
/// This function produces pretty-printed JSON output without span information.
///
/// # Examples
...
/// ```
#[allow(clippy::missing_panics_doc)]
pub fn to_string_compact_pretty<S>(syn: &S) -> String
where
    S: Syn,
{
    let adapter = syn.to_adapter();
